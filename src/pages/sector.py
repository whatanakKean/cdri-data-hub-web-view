        ...
        dmc.Paper([
            ...
            *([dmc.Select(
            ...
            dmc.Select(
                ...
                checkIconPosition="right",
                allowDeselect=False,
            )] if route == "/agriculture-and-rural-development" else []),
            *([dmc.Select(
            ...
            dmc.Select(
                ...
                checkIconPosition="right",
                allowDeselect=False,
            )] if route == "/development-economics-and-trade" else []),
            dmc.Select(
...
def sector_page(route):
    conn = sqlite3.connect("./src/data/data.db")
    
    if route == "/agriculture-and-rural-development":
        data = pd.read_sql_query(f"SELECT * FROM agriculture_data;", conn)
    elif route == "/development-economics-and-trade":
    ...
    return dmc.Container([
        dmc.Grid([
            dmc.GridCol(sidebar(data, route), span={"base": 12, "sm": 3}),
            dmc.GridCol([
                dmc.Stack([
                    dmc.Paper([
                        dmc.Tabs(
                            children=[
                                dmc.TabsList([
                                    dmc.TabsTab(f"Map View", leftSection=DashIconify(icon="tabler:map"), value="map"),
                                ...
                                dmc.TabsPanel(children=[html.Div(id='graph-id')], value="graph"),
